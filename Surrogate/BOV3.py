...
and normalization of data plus higher noise parameter
"""
...
def argmax_ei(gp, f_opt, scaler: StandardScaler):
    lb = scaler.transform(np.reshape([h_s0_range[0], h_s1_range[0], w_s0_range[0], w_s1_range[0]], (1, -1))).flatten()
    ub = scaler.transform(np.reshape([h_s0_range[1], h_s1_range[1], w_s0_range[1], w_s1_range[1]], (1, -1))).flatten()
    limits = np.vstack((lb, ub)).T
...
if __name__ == "__main__":
    ...
    for _ in range(max_iter):
        ...
        X_scaler.fit(X)
        f_scaler.fit(np.reshape(f, (-1,1)))

        X_scaled = X_scaler.transform(X)
        f_scaled = f_scaler.transform(np.reshape(f, (-1,1)))

        f_opt_scaled = np.max(f_scaled)

        # Fit model
        gp = fit_gp(X_scaled, f_scaled)

        # Find new point
        x_n_scaled, ei_max = argmax_ei(gp, f_opt_scaled, X_scaler)

        # For logging
        mu_x, sigma_x = gp.predict([x_n_scaled], return_std=True)

        # Sample objective function and append
        x_n = X_scaler.inverse_transform([x_n_scaled])
        ...
        # Log
        f_opt = f_scaler.inverse_transform([[f_opt_scaled]])
