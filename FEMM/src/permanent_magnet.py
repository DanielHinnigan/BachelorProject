...
def permanent_magnets():
    # Radius of inner points of magnets
    r = 30

    magnet_points = []
    for p in range(num_poles):
        # Lower corner of first magnet
        theta_A = (theta_pole-constants.theta_pm)/2+theta_pole*p
        x_A = r*np.cos(theta_A)
        y_A = r*np.sin(theta_A)

        x_B, y_B= sp.symbols("x_B, y_B")
        temp = sp.solve([x_B**2+y_B**2-r**2, (x_B-x_A)**2+(y_B-y_A)**2-pm_thickness**2], rational=False)
        x_B1 = float(temp[0][x_B])
        y_B1 = float(temp[0][y_B])
        x_B2 = float(temp[1][x_B])
        y_B2 = float(temp[1][y_B])

        x_L, y_L = (x_B1, y_B1) if (np.arctan2(y_B1,x_B1)%(2*np.pi)) < (np.arctan2(y_B2,x_B2)%(2*np.pi)) else (x_B2, y_B2)
        
        # Upper left corner of first magnet
        x_ul = (r+pm_width)*np.cos(theta_A)
        y_ul = (r+pm_width)*np.sin(theta_A)
        ...
        mi_drawline(x_ur, y_ur, x_L, y_L)
        mi_drawline(x_L,y_L,x_A,y_A)
        mi_drawline(x_A,y_A,x_ul,y_ul)

        # Add material to magnet
        midpoint_x = (x_ur+x_ul+x_L+x_A)/4
        midpoint_y = (y_ur+y_ul+y_L+y_A)/4
        ...
        if (p%2) == 0:
            # North pole
            mi_setblockprop("42SH", 0, 0, 0, theta_A/(2*np.pi)*360)
        else:
            mi_setblockprop("42SH", 0, 0, 0, theta_A/(2*np.pi)*360+180)

        # ------------------------------------- LOWER MAGNET -----------------------------------
        
        # Lower corner of second magnet
        theta_A = (theta_pole+constants.theta_pm)/2+theta_pole*p
        x_A = r*np.cos(theta_A)
        y_A = r*np.sin(theta_A)
        ...
        # Upper left corner of left magnet
        x_ul = (r+pm_width)*np.cos(theta_A)
        y_ul = (r+pm_width)*np.sin(theta_A)
        
        # Upper right corner of left magnet
        x_ur = x_ul + x_L-x_A
        y_ur = y_ul + y_L-y_A
...
def permanent_magnets_v2():
    ...
    for p in range(num_poles):
        ...
        mi_drawline(
            x_ul, 
            y_ul, 
            x_ul+constants.w_air_scaling*(x_ul-x_ll), 
            y_ul+constants.w_air_scaling*(y_ul-y_ll)
        )
        mi_drawline(
            x_ur, 
            y_ur, 
            x_ur+constants.w_air_scaling*(x_ur-x_lr), 
            y_ur+constants.w_air_scaling*(y_ur-y_lr)
        )
        mi_drawline(
            x_ul+constants.w_air_scaling*(x_ul-x_ll), 
            y_ul+constants.w_air_scaling*(y_ul-y_ll), 
